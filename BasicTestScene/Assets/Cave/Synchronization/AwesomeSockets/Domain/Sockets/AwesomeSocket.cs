...
namespace AwesomeSockets.Domain.Sockets
{
    public class AwesomeSocket : ISocket
    {
        ...
        {
            return InternalSocket.Receive(Buffer.GetBufferRef(buffer));
        }

        public int ReceiveMessage(string ip, int port, Buffer buffer)
        {
            EndPoint remoteEndPoint = new IPEndPoint(IPAddress.Parse(ip), port);
            return InternalSocket.ReceiveFrom(Buffer.GetBufferRef(buffer), ref remoteEndPoint);
        }
